...
**然后如果需要用到框架式访问对象，可以使用springContext的applicationContext，这里还有许多重要的生命周期扩展点**
...
2、springWeb(SpringMVC)：针对web的applicationContext，
...
ThreadLocalMap的数据结构，注意，并非是ThreadLocal的结构，不要混淆

<img src="format,png.png" alt="img" style="zoom:67%;" />



看代码，ThreadLocal是在某个类中的静态static的，所以每个线程都可以去获取这个类的ThreadLocal并使用add的。

线程1一开始add(里面是set)的时候，ThreadLocal的对象将放到你当前线程的ThreadLocalMap里，ThreadLocalMap的数据结构就是上图，对象将会进行两步走。

第一步，根据ThreadLocal对象的hash值，定位到ThreadLocalMap的table中的位置。(hash值这里就可以排除那些同类不同对象的对象)

第二步，看该位置是否为空，空则放入。如果不为空，则找下一个位置放入(同类不同对象的对象会放入下一个ThreadLocalMap的槽位)

线程1 ThreadLocal.get()取的时候，取的是线程1一开始add的值，一定要在remove之前取。



问题一：

如果线程2来取，是取不到的，因为本线程是没有进行add操作，所以是无法进行get操作。

问题二：

如果线程1一直进行，key又是弱引用被回收了，如果创建ThreadLocal的线程一直持续运行，那么这个Entry对象中的value就有可能一直得不到回收，发生内存泄露。

就比如线程池里面的线程，线程都是复用的，那么之前的线程实例处理完之后，出于复用的目的线程依然存活，所以，ThreadLocal设定的value值被持有，导致内存泄露。

按照道理一个线程使用完，ThreadLocalMap是应该要被清空的，但是现在线程被复用了。
remove是对当前线程ThreadLocalMap的删除
...
```
1.0.构造方法执行前-产前护理InstantiationAware -BeanPostProcessor
1.调用构造方法：我出生了！
1.2.构造方法执行后-产后护理InstantiationAware -BeanPostProcessor

1.3.属性赋值执行前-出生登记
2.设置属性：我的名字叫张铁钢

3.调用BeanName -Aware  #setBeanName方法:我要上学了，起了个学名
4.调用BeanFactory -Aware  #setBeanFactory方法：选好学校了

5.调用BeanPostProcessor#postProcessBeforeInitialization方法：到学校报名啦
...
## 2、springWeb(SpringMVC重点)

springMVC也是经常问的，经常会问到springmvc的工作流程，很多人可能会懵逼，因为平时都是用@RequestMapping，并没有真正去了解原理。
...
<img src="https://img-blog.csdnimg.cn/5162f08457da4f4f8a2a066d3f6e1d4b.png?x-oss-process=image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBA5Y2B5LiJ5bm65aSp6IOh4oCd,size_20,color_FFFFFF,t_70,g_se,x_16" alt="img" style="zoom:80%;" />

 1.用户向服务器发送请求，请求被Spring 前端控制Servelt **DispatcherServlet(中央处理器)**捕获；

2.DispatcherServlet对请求URL进行解析，得到请求资源标识符（URI）。然后根据该URI，**调用HandlerMapping获得该Handler配置的所有相关的对**象（包括Handler对象以及Handler对象对应的拦截器），最后以HandlerExecutionChain对象的形式返回给DispatcherServlet(中央处理器)；

**3.DispatcherServlet 会根据 处理器执行链 中的处理器，找到能够执行该处理器的处理器适配器(HandlerAdaptor)  --注，处理器适配器有多个。处理器适配器 (HandlerAdaptoer) 会调用对应的具体的 Controller（附注：如果成功获得HandlerAdapter后，此时将开始执行拦截器的preHandler(…)方法）**
...
**DispatcherServlet对请求URL进行解析，得到请求资源标识符（URI），DispatcherServlet 会维护一个key-value，key是uri，value是对应的controller，然后根据url选择不同的controller去执行，最后获得model对象，然后viewReslover结合它渲染视图返回给客户端**
